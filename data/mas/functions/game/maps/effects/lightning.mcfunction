...
execute at @e[type=minecraft:armor_stand,tag=mas.hunter_spawn,limit=1] unless entity @e[type=minecraft:armor_stand,tag=mas.lightning,limit=1] run summon armor_stand ~ ~ ~ {Marker:1b,Invisible:1b,Tags:["mas.lightning"]}

#SUMMON LIGHTNING
execute at @e[type=minecraft:armor_stand,tag=mas.lightning,limit=1] run summon lightning_bolt ~ ~ ~
