...
execute if score #game_state mas.counters = #NO_GAME mas.enums unless predicate mas:map/correct_players run tellraw @s ["",{"text":"[","bold":true,"color":"gray"},{"text":"MineAndSeek","bold":true,"color":"blue"},{"text":"]","bold":true,"color":"gray"},{"text":" This map does not support ","color":"red"},{"score":{"name":"#players","objective":"mas.counters"},"bold":true,"color":"dark_red"},{"text":" players.","color":"red"}]
execute if score #game_state mas.counters = #NO_GAME mas.enums if predicate mas:map/correct_players run function mas:game/state/start